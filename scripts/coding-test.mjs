...
const PERSONAS = {
  strong: `You are a strong candidate in a coding interview. You think out loud
BEFORE coding: state the brute force, its complexity, then the better approach.
You proactively mention edge cases and state time/space complexity. When you are
ready to write code, emit it. Keep spoken parts under 90 words.`,
  silent: `You are a competent but quiet candidate. You tend to jump straight to
code without explaining, and you only give complexity when explicitly asked.
Your first attempt has a subtle bug. Keep responses under 70 words.`,
};
...
Reply as JSON only: {"say": "<what you say out loud>", "code": "<full python source or null if unchanged>", "run": <true if you want to run the tests now>}
...
let r = await api("/api/loop", { company, level, rounds: ["coding"] });
console.log(`=== coding · ${company}/${level} · persona=${persona} ===`);
...
for (let turn = 0; turn < 8 && !r.done; turn++) {
  ...
  if (move.code) currentCode = move.code;
  history.push({ role: "candidate", text: move.say });
  console.log(`CANDIDATE: ${move.say}`);
  if (move.code) console.log(`  [wrote ${move.code.split("\n").length} lines]`);
  ...
  if (move.run) {
    ...
    const summary = run.compileError
      ? `ERROR: ${run.compileError.slice(0, 120)}`
      : `${run.passed}/${run.total} passed`;
    console.log(`  [RUN → ${summary}]`);
    history.push({ role: "candidate", text: `(I ran the tests: ${summary})` });
  }
  console.log();

  r = await api("/api/interview", {
    sessionId: r.sessionId,
    userMessage: move.say + (move.run ? " (I just ran the tests.)" : ""),
    artifact: { language: "python", code: currentCode },
  });
  history.push({ role: "interviewer", text: r.reply });
  console.log(`INTERVIEWER: ${r.reply}\n`);
}
